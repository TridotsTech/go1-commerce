...
def authorize_access(reauthorize=None):
	...
	redirect_uri = get_request_site_address(True) + "?cmd=go1_commerce.utils.google_indexing.google_callback"
...
def get_google_indexing_object():
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'go1_commerce.go1_commerce.get_google_indexing_object.get_google_indexing_object')
...
def publish_site(url, operation_type="URL_UPDATED"):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'go1_commerce.go1_commerce.get_google_indexing_object.publish_site')
...
def make_view_log(path, referrer=None, browser=None, version=None, url=None, user_tz=None):
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'go1_commerce.go1_commerce.get_google_indexing_object.make_view_log')
...
def is_tracking_enabled():
	try:
		...
		frappe.log_error(frappe.get_traceback(), 'go1_commerce.go1_commerce.get_google_indexing_object.is_tracking_enabled')
