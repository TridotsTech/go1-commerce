...
def get_category_brands_filter(product_ids):
	...
	if product_ids:
	    ...
	    products_filter = tuple(products_filter[:-1].split(',')) 
	    query = (
	        frappe.qb.from_(Product)
	        .join(ProductBrand)
	        .on(Product.brand == ProductBrand.name)
	        .where(Product.name.isin(products_filter))
	        .groupby(ProductBrand.name, ProductBrand.brand_name, ProductBrand.unique_name)
	        .select(
	            ProductBrand.name.as_('brand'),
	            ProductBrand.brand_name,
	            ProductBrand.unique_name,
	            frappe.qb.functions.Count(Product.name).as_('item_count')
	        )
	    )
