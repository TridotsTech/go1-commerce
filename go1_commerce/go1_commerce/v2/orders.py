                ...
                if orderDelivery:
                    ...
                    if assigned_drivers:
                        for assigned_driver in assigned_drivers:
                            frappe.db.sql("""UPDATE `tabDrivers` 
                                            SET working_status = 'Available', 
                                                driver_status = 'Online' 
                                            WHERE name = %(driver)s 
                                        """,{'driver':assigned_driver.driver})
                            frappe.db.commit()
...
def get_items(cart,order_sub_total,categories):
    if cart:
        items = frappe.db.sql("""SELECT C.name,C.product,C.quantity,C.attribute_description,C.price,
                                    C.total,C.product_name,I.price,I.stock,I.short_description,I.route,
                                    I.image AS image, (SELECT GROUP_CONCAT(category)
                                                        FROM `tabProduct Category Mapping`
                                                        WHERE parent = I.name) AS categories
                                FROM `tabCart Items` C, `tabProduct` I
                                WHERE I.name = C.product
                                    AND C.parent = %(parent)s
                                ORDER BY C.idx
                            """, {'parent': cart[0].name}, as_dict=True)
...
def fetch_cart_items(cart_parent, category_filter, category_info=None):
    ...
    items_query = f"""SELECT SUM(C.total) AS order_sub_total, C.product_name
                    FROM `tabCart Items` C
                    WHERE C.parent = "{cart_parent}"
                        AND C.product IN (SELECT parent
                                        FROM `tabProduct Category Mapping`
                                        WHERE category IN ({category_filter}))
                    ORDER BY C.idx """
    items = frappe.db.sql(items_query, as_dict=True)
    items_and_discounted_items['regular_items'] = items
    if category_info and category_info.minimum_order_amount > 0:
        discounted_items_query = """SELECT SUM(C.total) AS order_sub_total, C.product_name
                                    FROM `tabCart Items` C
                                    WHERE C.parent = "{parent}"
                                        AND C.product IN (SELECT parent
                                                            FROM `tabProduct Category Mapping`
                                                            WHERE category IN ({category_filter}))
                                        AND C.discount_rule IS NOT NULL
                                    ORDER BY C.idx
                                """.format(parent = cart_parent, category_filter = category_filter)
        discounted_items = frappe.db.sql(discounted_items_query, as_dict = True)
        if discounted_items:
...
def validate_order_pincode_validate(ship_addr):
    customer_shipping_address = frappe.db.sql(""" SELECT *
                                                ...
                                                """,{'addressId': ship_addr},as_dict=True)
        ...
        if category:
            ...
            return frappe.db.sql("""SELECT name
                                    ...
                                    WHERE
                                        ...
                                        AND lft >= {lft}
                                        AND rgt <= {rgt}
                                """.format(lft=lft, rgt=rgt), as_dict=True)
    except Exception:
    ...
    try:
        ...
        if products:
            ...
            if customer_id:
                ...
                if cart:
                    condition = ''
                    condition += ' I.name = C.product AND C.parent = %(parent)s AND C.product = %(productid)s'
                    if attributeId:
                        condition += ' AND C.attribute_ids = %(attr_id)s'
                    query = ''' SELECT C.name, C.product, C.attribute_ids, C.quantity, C.attribute_description,
                                    C.price, C.total, C.product_name, I.price, I.stock, I.short_description,
                                    I.route,I.image, I.maximum_order_qty
                                FROM
                                    `tabCart Items` C
                                INNER JOIN
                                    `tabProduct` I ON C.product = I.name
                                WHERE {condition}
                                ORDER BY C.idx '''.format(condition = condition)
                    cart_items = frappe.db.sql(query, {'parent': cart[0].name,
                                                        'productid': ProductId,
                                                        'attr_id': attributeId}, as_dict=1)
                    if cart_items:
...
def validate_insert_order(res,order_settings,cart,customer_id,catalog_settings,request,customers):
    try:
        ...
        if cart:
            ...
            for cart_item in items:
                ...
                if cart_item.attribute_ids:
                    ...
                    combination_weight = frappe.db.sql('''	SELECT weight, sku
                                                            FROM `tabProduct Variant Combination`
                                                                WHERE parent = %(name)s AND
                                                                attributes_json = %(attribute_id)s''',
        ...
        payment_method = None
        if order_id:
            frappe.db.sql("""UPDATE `tabOrder` set payment_status ='Pending' WHERE
            name=%(name)s""",{'name':order_id})
...
def set_address_details(request,order,customer_info):
    ...
    if request.get("bill_addr"):
        customer_billing_address = frappe.db.sql('''SELECT *
                                                    FROM
                                                        `tabCustomer Address`
                                                    WHERE name = %(addressId)s
                                                    ''',{'addressId': request.get('bill_addr')}, 
                                                        as_dict = True)
    ...
    if request.get("ship_addr"):
        customer_shipping_address = frappe.db.sql('''	SELECT * FROM `tabCustomer Address`
                                                        WHERE name = %(addressId)s
                                                        ''', {'addressId': request.get('ship_addr')},
                                                            as_dict=True)
        if customer_shipping_address:
...
def get_item(cart,order_sub_total,order_total,total_weight,items):
    if cart:
        items = frappe.db.sql( '''SELECT C.name, C.product, C.special_instruction,
                                        C.quantity, C.attribute_description, C.price,
                                        C.shipping_charges, C.product_name, C.tax,
                                        I.stock, I.short_description, I.route,I.image,
                                        C.is_free_item, C.discount_rule, C.attribute_ids, C.total
                                FROM
                                    `tabCart Items` C
                                INNER JOIN
                                    `tabProduct` I
        ...
        checkout_attributes = frappe.db.sql(''' SELECT *
                                                FROM `tabCart Checkout Attributes`
...
def get_shipping_methods(request,order,order_id,order_total):
    ...
    if order_id:
        ...
        for x in check_attributes:
            frappe.db.sql(''' DELETE FROM `tabOrder Checkout Attributes` WHERE
                                name = %(id)s ''',{"id":x.name})
            frappe.db.commit()
...
def get_items_item(cart):
    items = frappe.db.sql('''SELECT C.name, C.product, C.checkout_attributes,
                                C.special_instruction, C.quantity,
                                C.price, C.total, C.shipping_charges,
                                C.attribute_ids, I.stock, I.short_description,
                                C.is_gift_card, C.sender_name, C.sender_email,
                                C.recipient_name, C.recipient_email,
                                C.tax,C.product_name,C.attribute_description,
                                C.certificate_type, C.certificate_occasion,
                                I.image AS image, C.sender_message,I.route,
                                C.is_free_item, C.is_gift_card, C.discount_rule
                            FROM
                                `tabCart Items` C, `tabProduct` I
                            WHERE
                                I.name = C.product AND C.parent = %(parent)s
                            ORDER BY C.idx''',
                                {'parent': cart[0].name}, as_dict=True)
...
def checkout_attributes_html(request,cart,order_info,cart_item):
    if not request.get("checkout_attributes_html"):
        checkout_attributes = frappe.db.sql('''SELECT * FROM
                                                `tabCart Checkout Attributes` where
                                                parent=%(parent)s ORDER BY idx''',
                                                {'parent': cart[0].name}, as_dict=1)
        if checkout_attributes:
            cart_idx = 1
...
def set_customer_shipping_address(request,order):
    customer_shipping_address = frappe.db.sql("""SELECT *
                                                ...
                                                WHERE
                                                    name = %(addressId)s""",
                                            {'addressId': request.get('ship_addr')["name"]}, as_dict=True)
    if customer_shipping_address:
        order.shipping_first_name = customer_shipping_address[0].first_name
        order.shipping_city = customer_shipping_address[0].city
        order.shipping_zipcode = customer_shipping_address[0].zipcode
    ...
    try:
        ...
        if customer_id:
            ...
            from go1_commerce.go1_commerce.v2.product \
                import get_attributes_combination
            ...
            eligible_items = get_eligible_items(condition = condition, orderid = order_id,
...
def get_eligible_items(condition, orderid, wherecondition):
    query = f'''SELECT O.name, O.item AS product_id, O.item_name, O.attribute_ids, O.item_sku AS sku,
                    P.brand, P.brand_name, O.quantity
                FROM
                    `tabOrder Item` O
                INNER JOIN
                    `tabOrder` OD ON OD.name = O.parent
                INNER JOIN
                ...
                WHERE
                    ...
                    {wherecondition}
                GROUP BY
                    O.attribute_ids, O.item'''
    eligible_items = frappe.db.sql(query,as_dict=1)
...
def calculate_fixed_rate_shipping_charges(shipping_method,shipping_addr,shopping_cart):
    ...
    if sh_status.get("status") == "success":
        ...
        if order_subtotal:
            sh_list = frappe.db.sql(""" SELECT shipping_zone,charge_amount
...
def calculate_total_based_shipping_charges(shipping_method,shipping_addr,shopping_cart):
    ...
    if sh_status.get("status") == "success":
        ...
        if order_subtotal:
            sh_list = frappe.db.sql(f""" SELECT shipping_zone,use_percentage,charge_amount,charge_percentage
                                        FROM
                                            `tabShipping By Total Charges`
                                        WHERE
                                            shipping_method = "{shipping_method}"
                                            AND '{order_subtotal}' >= order_total_from
                                        AND '{order_subtotal}' <= order_total_to """, as_dict = True)
            if sh_list:
...
def calculate_weight_based_shipping_charges(shipping_method,shopping_cart):
    ...
    if sh_status.get("status") == "success":
        ...
        if order_subtotal:
            sh_list = frappe.db.sql(""" SELECT shipping_zone,use_percentage,charge_amount,charge_percentage
                                        FROM
                                            `tabShipping By Weight Charges`
                                        WHERE
                                            shipping_method = %(shipping_method)s
                                            AND %(weight)s >= order_weight_from
                                            AND %(weight)s <= order_weight_to 
                                    """,{'weight': total_weight,'shipping_method': shipping_method}, 
                                        as_dict = True)
            if sh_list:
                ...
                if not free_shipping:
                    if item.attribute_ids and has_variants:
                        ...
                        if item.attribute_ids:
                            ...
                            for item in attr_id:
                                attribute_weight = frappe.db.sql(''' SELECT weight_adjustment 
                                                                    FROM `tabProduct Attribute Option` 
                                                                    WHERE name = "{name}"
                                                                '''.format(name = item),as_dict = 1)
                                if attribute_weight and attribute_weight[0].weight_adjustment:
...
def get_product_variant_weight(item,attribute_id):
    ...
    while loop < 2:
        combination_weight = frappe.db.sql(	""" SELECT weight 
                                            ...
                                            """,{'name': item.product,'attribute_id': attribute_id}, 
                                                as_dict = True)
...
def calculate_get_dbased_shipping_charges(shipping_method, shipping_addr, subtotal,shipping_charges):
    ...
    if distance_info.get("status") == "success":
        ...
        shipping_charges_list = frappe.db.sql(f"""  SELECT use_percentage,charge_percentage,charge_amount
                                                    FROM
                                                        `tabShipping Charges By Distance`
                                                    WHERE
                                                        shipping_method = "{shipping_method}"
...
def calculate_distance_total_based_shipping_charges(shipping_method,shipping_addr,shopping_cart):
    ...
    if sh_status.get("status") == "success":
        ...
        if distance_info.get("status") == "success":
            distance_value = distance_info.get("distance_value")
            distance = round((distance_value * 0.001),2)
            s_query = """ SELECT use_percentage,charge_percentage,charge_amount
                        FROM `tabShipping Charges By Distance and Total` SC
                        INNER JOIN `tabShipping Rate Method` SR ON SR.name = SC.parent
                        WHERE
                            shipping_method='{shipping_method}' 
                            AND {distance} >= from_distance
                            AND {distance} <= to_distance 
                            AND {order_subtotal} >= order_total_from
                            AND {order_subtotal} <= order_total_to
                    """.format(order_subtotal = order_subtotal,distance = distance,
                               shipping_method = shipping_method)
            shipping_charges_list = frappe.db.sql(s_query,as_dict=True)
            if shipping_charges_list:
                if shipping_charges_list[0].use_percentage:
    ...
    if sh_status.get("status") == "success":
        ...
        if distance_info.get("status") == "success":
            ...
            shipping_charges_list = frappe.db.sql("""SELECT SC.use_percentage,SC.charge_percentage,
                                                            SC.charge_amount
                                                    FROM `tabShipping Charges By Distance And Weight` SC
                                                        INNER JOIN `tabShipping Rate Method` SR 
                                                            ON SR.name = SC.parent
                                                    WHERE
                                                        shipping_method=%(shipping_method)s 
                                                        AND %(distance)s >= from_distance
                                                        AND %(distance)s <= to_distance 
                                                        AND %(weight_total)s >= order_weight_from
                                                        AND %(weight_total)s <= order_weight_to
...
def get_sh_chrgs_of_distance_and_weight_group(distance,shipping_rate_method,weight_total,
    ...
    shipping_chrgs_km_kg = frappe.db.get_value("Shipping Rate Method", 
                                               ...
                                               as_dict = 1)
    weight_charges_list = frappe.db.sql(f"""SELECT charge_amount,order_weight_from,order_weight_to
                                            FROM
                                                `tabShipping By Weight Charges`
                                            WHERE
                                                parent = '{shipping_rate_method}'
                                            AND '{weight_total}' >= order_weight_from
                                            AND '{weight_total}' <= order_weight_to  """,as_dict = True)
    if weight_charges_list:
        ...
        weight_charges_list_all = frappe.db.sql(f""" SELECT order_weight_to,charge_amount 
                                                    FROM `tabShipping By Weight Charges`
                                                    WHERE
                                                        parent = '{shipping_rate_method}'
                                                        AND order_weight_to <= '{weight_total}'
                                                    ORDER BY order_weight_to 
                                                    LIMIT 1 
                                                """, as_dict = True)
    ...
    distance_charges_list = frappe.db.sql(
                        f"""SELECT charge_amount,to_distance
                            FROM `tabShipping Charges By Distance`
                            WHERE
                                parent = '{shipping_rate_method}' 
                                AND '{distance}' >= from_distance 
                                AND '{distance}' <= to_distance 
    ...
    if distance_charges_list:
        ...
        distance_charges_list_all = frappe.db.sql(f""" SELECT charge_amount, to_distance
                                                        FROM `tabShipping Charges By Distance`
                                                        WHERE
                                                            parent = '{shipping_rate_method}'
                                                                AND to_distance <= '{distance}' 
                                                        ORDER BY to_distance ASC 
                                                    """, as_dict = True)
...
def validate_product_cart_qty_conditions(cart,product,ProductId,attributeId,add_qty,qty,customer):
    condition = f' I.name = C.product AND C.parent = "{cart[0].name}" AND C.product = "{ProductId}"'
    if attributeId:
        condition += f' AND C.attribute_ids = "{attributeId}"'
    query = f'''SELECT C.name, C.product, C.attribute_ids, C.quantity, C.attribute_description,
                    C.price, C.total, C.product_name, I.price, I.stock, I.short_description,
                    I.route, I.image, I.maximum_order_qty
                FROM
                    `tabCart Items` C, `tabProduct` I
                ...
                ORDER BY C.idx
            '''
    cart_items = frappe.db.sql(query, as_dict=1)
    if cart_items:
        cart_qty = sum(i.quantity for i in cart_items)
        if add_qty == 'false':
            cart_qty = cart_qty
...
def edit_order(order_id):
    ...
    if customer and customer == order.customer:
        old_items = frappe.db.sql(	"""SELECT TM.item, TM.quantity, TM.attribute_ids
                                        FROM `tabOrder Item` TM
                                        WHERE
                                            TM.parent = %s 
...
def get_wallet_list(cart,order_id):
    frappe.db.sql("""DELETE FROM
                        `tabCart Items` CI
                    WHERE
                        CI.parent = %s """, (cart[0].name,))
    wallet_list = frappe.db.sql("""SELECT WT.name
        ...
        if check_order and customer:
            ...
            if cart:
                ...
                frappe.db.sql('''DELETE
                                FROM `tabCart Items` WHERE parent=%(parent)s ''',
                                {'parent': cart[0].name})
                frappe.db.commit()
                return {
                        "status":"Success",
...
def get_cart_items(order_id,customer):
    ...
    query2 = """DELETE 
                FROM `tabDiscount Usage History` DUH
                WHERE DUH.order_id = '{0}'
            """.format(order_id)
    result2 = frappe.db.sql(query)
    order_info = frappe.get_doc('Order', order_id)
    cart = frappe.db.get_all('Shopping Cart',
        filters={'customer': customer, 'cart_type': 'Shopping Cart'},
        fields=['name', 'tax', 'tax_breakup', 'total'])
...
def order_items_list(cart):
    items = frappe.db.sql(	'''SELECT C.name, C.product, C.checkout_attributes, C.special_instruction,
                                    C.quantity, C.attribute_description, C.price, C.total,
                                    C.shipping_charges,I.short_description, I.route,
                                    C.recipient_name, C.recipient_email,C.certificate_occasion,
                                    C.product_name, C.tax, C.attribute_ids, I.stock,
                                    C.is_gift_card, C.sender_name, C.sender_email,
                                    C.sender_message, C.certificate_type,
                                    I.image AS image, C.is_free_item, C.is_gift_card, C.discount_rule
                                FROM
                                    `tabCart Items` C, `tabProduct` I
                                WHERE
                                    I.name = C.product AND C.parent = %s
                                ORDER BY
                                    C.idx
                            ''', (cart[0].name,), as_dict=True)
    return items
    ...
    if cart_item.attribute_ids:
        ...
        combination_weight = frappe.db.sql('''  SELECT weight,sku 
                                                ...
                                                WHERE parent=%(name)s 
                                                    AND attributes_json=%(attribute_id)s
                                            ''',{'name': cart_item.product,
                                                'attribute_id': attribute_id.replace(" ","")}, as_dict=1)
        if combination_weight:
            item_weight = combination_weight[0].weight
            item_sku = combination_weight[0].sku
    else:
...
def get_checkout_attributes():
	try:
		checkout_attribute = frappe.db.sql('''SELECT C.question, C.control_type, C.is_required, C.name, 
                                                    C.is_group,C.charge_based_on_weight, C.short_description 
                                                FROM `tabCheckout Attributes` C 
                                                WHERE C.is_active = 1 
                                                    AND (C.is_group = 1 OR C.parent_attribute is null)
                                            ''',as_dict=1)
		if checkout_attribute:
			for item in checkout_attribute:
				item.options = frappe.db.sql('''SELECT O.option_value, O.display_order, O.price_adjustment,
                                                    O.name,O.is_pre_selected 
                                                FROM `tabCheckout Attributes Options` O 
                                                WHERE O.parent = "{name}" 
                                                ORDER BY O.display_order ASC
                                            '''.format(name=item.name),as_dict=1)
				if item.is_group:
					child_attributes = frappe.db.sql('''SELECT C.question, C.control_type, C.is_required, 
                                                            C.name,C.charge_based_on_weight, 
                                                            C.parent_attribute, C.parent_attribute_option, 
                                                            C.short_description 
                                                        FROM `tabCheckout Attributes` C
                                                        WHERE C.is_active = 1 
                                                            AND C.is_group = 0 
                                                                AND C.parent_attribute = %(parent)s
                                                    ''', {'parent': item.name},as_dict = 1)
					for ch in child_attributes:
						ch.options = frappe.db.sql('''SELECT O.option_value, O.display_order, 
                                                            O.price_adjustment, O.name,O.is_pre_selected 
                                                        FROM `tabCheckout Attributes Options` O
                                                        WHERE O.parent = "{name}" 
                                                        ORDER BY O.display_order ASC
                                                    '''.format(name = ch.name),as_dict = 1)
    ...
    for x in order.order_item:
        if x.is_free_item != 1:
            ...
            if not x.attribute_ids:
                p_price = frappe.db.sql(""" SELECT PP.price
                                            FROM `tabProduct` PP
                                            WHERE PP.name=%(product_id)s
                                            ORDER BY PP.price 
                                        """,{"product_id":x.item}, as_dict = 1)
                if not p_price:
                    no_stock = 1
                    is_allowed = 0
                if p_price:
...
def get_order_item_details(order_item):
    query = f""" SELECT O.customer,O.customer_email,O.customer_name,
                        OI.item product,OI.item_name product_name,OI.attribute_description,
                        OI.attribute_ids AS attribute_id
...
def create_return_shipment(dt, dn, products, driver, status, tracking_number=None, tracking_link=None):
	try:
		...
		if not order_shipmemt:
			...
			if driver:
				frappe.db.sql('''UPDATE `tabReturn Shipment` 
								SET driver = %(driver)s 
								WHERE name = %(orderid)s
							''',{'driver':driver,'orderid':order_shipmemt[0].name})
	except Exception:
...
def set_quantity_variants(doc, quantity, allow = 0):
    ...
    for item in order_items:
        ...
        if has_variant:
            ...
            variant_stock = frappe.db.sql(query, params, as_dict = True)[0]
            updated_quantity = 0
            if allow == 1:
