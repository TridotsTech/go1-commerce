...
def make_tax_templates():
	...
	tax_template = make_tax_template('Test Product Tax 1', business1)

def make_tax_template(tax_name, business):
	if not frappe.db.get_value('Product Tax Template', {'title': tax_name, 'business': business}):
		if not frappe.db.get_value('Product Tax Template', {'title': tax_name}):
			...
			if len(installed_apps)>0:
				if not frappe.db.get_value('Account', {'account_name': "Test Account 1", 'business': business}):
					account = frappe.get_doc({
						'doctype': "Account",
						'account_name': "Test Account 1",
						'business': business
					}).insert()
				account_id = frappe.db.get_value('Account', {'account_name': "Test Account 1", 'business': business}, "name")
				account_name = frappe.db.get_value('Account', {'account_name': "Test Account 1", 'business': business}, "account_name")
