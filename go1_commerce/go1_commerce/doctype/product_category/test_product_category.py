...
class TestProductCategory(unittest.TestCase):
	def test_category_creation(self):
		...
		cat1 = make_product_category("Test Product Category 1", business1)
		cat2 = make_product_category("Test Product Category 2", business2)
		cat1_doc = frappe.get_doc("Product Category", cat1).reload()
		cat2_doc = frappe.get_doc("Product Category", cat2).reload()
		
		

def make_product_category(category_name, business=None):
	if not frappe.db.get_value("Product Category", { "category_name": category_name, "business":business}):
		cat = frappe.get_doc({
		...
		return frappe.get_value("Product Category", { "category_name": category_name, "business":business}, "name")
