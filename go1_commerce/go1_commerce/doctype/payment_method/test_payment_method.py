...
class TestPaymentMethod(unittest.TestCase):
	def test_payment_creation(self):
		...
		pay1 = make_payment_method("Test Payment Method 1", business1)
		pay2 = make_payment_method("Test Payment Method 2", business2)
		pay1_doc = frappe.get_doc("Payment Method", pay1).reload()
		pay2_doc = frappe.get_doc("Payment Method", pay2).reload()
		
		

def make_payment_method(payment_method, business=None):
	if not frappe.db.get_value("Payment Method", { "payment_method": payment_method, "business":business}):
		pay = frappe.get_doc({
		...
		return frappe.get_value("Payment Method", { "payment_method": payment_method, "business":business}, "name")
