...
frappe.require("assets/ecommerce_business_store/css/toggle-slider.css");

frappe.ui.form.on('Shipping Cities', {
	refresh: function(frm) {
		...
		$('button[data-fieldname="add_area"]').css({"background":"#1b8fdb", "color":"white",
                                            "padding":"5px 10px","float":"right","position":"absolute",
                                            "right":"15px","top":"-35px"});
	...
	get_all_areas: function(frm){
		frappe.call({
            method: "ecommerce_business_store.ecommerce_business_store.doctype.shipping_cities.shipping_cities.get_areas_fromziprange",
            args: {
	...
	add_newarea: function(frm){
		window.open(window.location.origin+"/desk#Form/Area/New%20Area%201", '_blank');
	},
        ...
        $.each(possible_val, function(i, c) {
            ...
            $.ajax({
                ...
                success: function(r) {
                    ...
                    var drp_html = `<button class="btn btn-default btn-xs" id="add_newarea" 
                                        style="background: rgb(240, 244, 247); color: #36414c; 
                                        padding: 5px 10px; float: right;position:absolute;top:-55px; 
                                        right: 132px;">Add New Area
                                    </button>
                                    <div class="${c.cls}" style="padding: 0px;"> 
                                        ...
                                        <ul role="listbox" id="assets" class= "assets" style="list-style-type: none;
                                            position: absolute;width: 43%;margin: 0;background: rgb(255, 255, 255);
...
function area_dialog(frm) {
    frm.possible_val = [{
        ...
        "label": "Choose Area",
        "doctype": "Shipping Cities",
        "business": cur_frm.doc.business,
        "reference_doc": "Area",
        "reference_fields": escape(JSON.stringify(["name", "area", "city", "zipcode"])),
        "search_fields": "area",
        "reference_method": "ecommerce_business_store.ecommerce_business_store.doctype.shipping_cities.shipping_cities.get_area_list",
        "is_child": 0,
    ...
    categoryDialog.set_primary_action(__('Submit'), function() {
        ...
        if (cat_json.length <= 0) {
            frappe.throw(__('Please select any one area.'))
        } else {
    ...
    categoryDialog.set_secondary_action(__('Add new Area'), function() {
    })
