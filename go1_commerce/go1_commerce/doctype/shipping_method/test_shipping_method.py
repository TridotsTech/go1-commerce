...
class TestShippingMethod(unittest.TestCase):
	def test_shipping_creation(self):
		...
		ship1 = make_shipping_method("Test Shipping Method 1")
		ship2 = make_shipping_method("Test Shipping Method 2")
...
def make_shipping_method(shipping_method_name, business=None):
	if not frappe.db.get_value("Shipping Method", { "shipping_method_name": shipping_method_name}):
		ship = frappe.get_doc({
		...
		return frappe.get_value("Shipping Method", { "shipping_method_name": shipping_method_name}, "name")
