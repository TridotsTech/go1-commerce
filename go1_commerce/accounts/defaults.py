...
@frappe.whitelist()
def get_all_linked_docs(doctype, name, docs=None, visited=None):
	"""
	Get all nested submitted linked doctype linkinfo
	...
	linkinfo = get_linked_doctypes(doctype)
	linked_docs = getdoc_linked_docs(doctype, name, linkinfo)
	link_count = 0
	visited[doctype].append(name)
	for link_doctype, link_names in linked_docs.items():
		for link in link_names:
...
@frappe.whitelist()
def delete_all_linked_docs(doctype, name, docs=None, visited=None):
	"""
	Get all nested submitted linked doctype linkinfo
	...
	linkinfo = get_linked_doctypes(doctype)
	linked_docs = delete_linked_docs(doctype, name, linkinfo)
	link_count = 0
	visited[doctype].append(name)
	# sort linked documents by ascending number of links
	docs.sort(key=lambda doc: doc.get("link_count"))
	...
	for dt, link in linkinfo.items():
		...
		if not linkmeta.get("issingle"):
			...
			try:
				...
				# 		# ret = frappe.get_list(doctype=dt, fields=fields,
				# 		# 	filters=[[dt, "name", '=', me.parent]])
				# 		# query = '''update `tab{doctype}` set {setfield}="" where {confield}="{fieldval}"'''.format(doctype=dt, setfield=link.get("fieldname")[0], confield=link.get("fieldname")[0], fieldval=name)
				# 		# print(query)
...
def validate_linked_doc(docinfo):
	...
	if docinfo.get('doctype') in auto_cancel_exempt_doctypes:
		if docinfo.get('doctype') !="Vendor Orders":
			return False
